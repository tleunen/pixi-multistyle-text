...
export default class MultiStyleText extends PIXI.Text {
  ...
  constructor(text: string, styles: TextStyleSet) {
    ...
    INTERACTION_EVENTS.forEach(event => {
      this.on(event, (e: PIXI.interaction.InteractionEvent) =>
  ...
  private handleInteraction(e: PIXI.interaction.InteractionEvent) {
    ...
    let targetTag = this.hitboxes.reduce(
      (prev, hitbox) =>
        prev !== undefined
          ? prev
          : hitbox.hitbox.contains(localPoint.x, localPoint.y)
          ? hitbox
          : undefined,
      undefined
    );
  ...
  public set styles(styles: TextStyleSet) {
    ...
    this.textStyles["default"] = this.assign(
      {},
    ...
    this.withPrivateMembers()._style = new PIXI.TextStyle(
              ...
              Object.entries(this.textStyles[matches[j][1]]).forEach(style => {
                bbStyle[style[0]] =
                  typeof style[1] !== "string"
                    ? style[1]
                    : bbcodeTags[1] + style[1];
              });
              styleStack.push(
                this.assign({}, styleStack[styleStack.length - 1], bbStyle)
              );
            } else {
              styleStack.push(
                this.assign(
  ...
  public updateText(): void {
      ...
      for (let j = 0; j < outputTextData[i].length; j++) {
        ...
        if (outputTextData[i][j].text.length !== 0) {
          outputTextData[i][j].width +=
            (outputTextData[i][j].text.length - 1) * sty.letterSpacing;
      ...
      for (let j = 0; j < line.length; j++) {
        ...
        switch (style.valign) {
          ...
          default:
            ...
            linePositionY +=
              lineYMaxs[i] - fontProperties.ascent - style.valign;
            break;
        }
    ...
    drawingData.forEach(({ style, text, x, y }) => {
      ...
      this.context.shadowOffsetX =
        Math.cos(style.dropShadowAngle) *
        style.dropShadowDistance *
        this.resolution;
      this.context.shadowOffsetY =
        Math.sin(style.dropShadowAngle) *
        style.dropShadowDistance *
        this.resolution;
    ...
    drawingData.forEach(
      ({ style, text, x, y, width, ascent, descent, tag }) => {
    ...
    drawingData.forEach(
      ({ style, text, x, y, width, ascent, descent, tag }) => {
        let offset =
          -this.withPrivateMembers()._style.padding -
        ...
        if (debugSpan) {
          ...
          if (MultiStyleText.debugOptions.spans.bounding) {
            ...
            this.context.strokeStyle =
              MultiStyleText.debugOptions.spans.bounding;
          ...
          if (MultiStyleText.debugOptions.spans.baseline) {
            this.context.strokeStyle =
              MultiStyleText.debugOptions.spans.baseline;
            this.context.beginPath();
      ...
      for (let j = 0; j < tagSplit.length; j++) {
        if (re.test(tagSplit[j])) {
          ...
          for (let k = 0; k < words.length; k++) {
            ...
            if (
              this.withPrivateMembers()._style.breakWords &&
              wordWidth > spaceLeft
            ) {
              ...
              for (let c = 0; c < characters.length; c++) {
                const characterWidth = this.context.measureText(characters[c])
                  .width;
  ...
  protected updateTexture() {
    ...
    texture.baseTexture.setRealSize(
      this.canvas.width,
      this.canvas.height,
      this.resolution
    );
    texture.trim.width = texture.frame.width =
      this.canvas.width / this.resolution;
    texture.trim.height = texture.frame.height =
      this.canvas.height / this.resolution;

    texture.trim.x =
      -this.withPrivateMembers()._style.padding - dropShadowPadding;
    texture.trim.y =
      -this.withPrivateMembers()._style.padding - dropShadowPadding;

    texture.orig.width =
      texture.frame.width -
      (this.withPrivateMembers()._style.padding + dropShadowPadding) * 2;
    texture.orig.height =
